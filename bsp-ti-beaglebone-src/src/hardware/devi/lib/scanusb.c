...
static USBKCode aScanToUSB[] = {
   ...
   KS_o,          KS_p,          KS_bracketleft,KS_bracketright,KS_Enter,    KS_Control_L,   KS_a,         KS_s,
   ...
   KS_KP_Up,      KS_KP_Prior,   KS_KP_Subtract,KS_KP_Left,    KS_KP_Begin,  KS_KP_Right,    KS_KP_Add,    KS_KP_End, 
   ...
   KS_Lang3,      KS_Internat4,  KS_None,       KS_Internat5,  KS_None,      KS_Internat3,   KS_KP_Comma,  KS_None,        
...
static scanToUSB_Exception excTable[] = {
   { "\x2b"    , KS_NonUSNumsign}, /* This is an Euro keyboard exception: the same scan code as KS_backslash      */
				   ...
				   /* codes                                                                       */
   { "\xe0\x1c", KS_KP_Enter    },
   { "\xe0\x1d", KS_Control_R   },
   { "\xe0\x2a", KS_None        }, /* Dead scan code  */
   { "\xe0\x35", KS_KP_Divide   },
   { "\xe0\x37", KS_Print_Screen}, /* Actualy it is 0xe02ae037 (make) and 0xe0b7e0aa (breake);    */
				   /* we drop 0xe0aa and 0xe02a because these are dead scan codes */
   { "\xe0\x38", KS_Alt_R       },
   { "\xe0\x46\xe0\xc6", KS_Pause},
   { "\xe0\x47", KS_Home        },
   { "\xe0\x48", KS_Up          },
   { "\xe0\x49", KS_PgUp        },
   { "\xe0\x4b", KS_Left        },
   ...
   { "\xe0\x5b", KS_LeftGUI     },
   { "\xe0\x5c", KS_RightGUI    },
   { "\xe0\x5d", KS_Application },
   { "\xe1\x1d\x45\xe1\x9d\xc5", KS_Pause}, 
/* Dead scan codes                                                                  */   
   { "\xe0\xaa", KS_None        },
   { "\xe0\xb6", KS_None        },
   { "\xe0\x36", KS_None        }
...
static USBKCode getUSBCodeFromScanCodes(BYTE * nScanCodes, int nNumScanCodes);
static USBKCode searchExceptionTable(BYTE * nScanCodes, int nNumScanCodes, int * pnCompareLength);
...
USBKCode getUSBCodeFromScanCodes(BYTE * nScanCodes, int nNumScanCodes)
{
...
USBKCode searchExceptionTable(BYTE * nScanCodes, int nNumScanCodes, int * pnCompareLength)
{
   int i, j, k;
...
int devi_scan_to_usb(unsigned char * buf, int * pnBytes, USBKCode * usbCodes, unsigned char * makeBreakes, int * pnLen)
{
   int nPos = 0;
   int nUsbPos = 0;
   USBKCode code;
   ...
   while(nPos < *pnBytes)
	...
	(0xe1 == buf[nPos]))
	  ...
	  *(buf + nPos + 1) &= 0x7f;                      // We don't need any more make/break flag
	  code = searchExceptionTable(buf + nPos, *pnBytes - nPos, &nCompareLength); 
...
int devi_usb_to_scan(USBKCode usbCode, int nMakeBreak, _uint8 * buf, int * pnLen)
{
   ...
   for(i = 0; i <  ARRAY_SIZE(excTable); ++i)
     ...
     if(excTable[i].USBCode == usbCode)
	  {
	  int nLen = strlen(excTable[i].pScanCodes);
	  if(nLen > *pnLen)
	     return ERANGE;
	  strcpy(buf, excTable[i].pScanCodes);
