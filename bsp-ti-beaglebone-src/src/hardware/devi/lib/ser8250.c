...
static int uart_pulse(message_context_t *ctx, int code, unsigned flags, void *data);
...
int is_8250(struct private_data *dp) 
{
...
/* Output     : None                                                                */
/* Return     : None                                                                */
void set_8250(struct private_data *dp, unsigned baud, tcflag_t cflag) 
{
	unsigned port = dp->port;
...
/* Output     : None                                                                */
/* Return     : None                                                                */
void reset_8250(struct private_data *dp, unsigned msec) 
{
	unsigned port = dp->port;
...
/* Output     : None                                                                */
/* Return     : None                                                                */
void enable_8250(struct private_data *dp) 
{
	unsigned port = dp->port;
...
int uart_init(input_module_t *module)
{
	...
	if(!module->data) {
		...
		dp->irq = 4;
		dp->trigger_level = 1;
	        dp -> priority = min((getprio(0) + PROMOTE_PRTY), sched_get_priority_max(sched_getscheduler(0)));
	}
...
/* Output     : None                                                                */
/* Return     : EOK if OK, otherwise -1                                               */
int uart_devctrl(input_module_t *module, int event, void *ptr)
{
	struct private_data *dp = module->data;
...
/* Output     : None                                                                */
/* Return     : EOK if OK, otherwise -1                                               */
int uart_pulse(message_context_t *ctx, int code, unsigned flags, void *data)
{
...
/* Output     : None                                                                */
/* Return     : EOK if OK, otherwise -1                                             */
int uart_reset(input_module_t *module)
{
	struct private_data		*dp = module->data;
        int                             i;
	...
	if((dp->flags & FLAG_INIT) == 0) {
		...
		dp->pc = devi_register_pulse(module, DEVI_PULSE_ALLOC, 
					     uart_pulse); 
		if (dp->pc < 0) {
		...
		memset(&dp->ev, 0, sizeof(dp -> ev));
	           ...
	           {
                   SIGEV_PULSE_INIT(&(dp -> ev[i]), devi_get_coid(), dp -> priority, NO_PULSE_ASSIGNED, 0);
	           }
...
/* Output     : None                                                                */
/* Return     : always EOK                                                          */
int uart_output(input_module_t *module, void *p, int num)
{
	struct private_data 	*dp = module->data;
	char 			kick = 0;
	int			i = 0;
...
int uart_parm(input_module_t *module, int opt, char *optarg)
{
	struct private_data *dp = module->data;
	...
	case 'P':
		  ...
		  if((nPrty > 0) && (nPrty <= sched_get_priority_max(sched_getscheduler(0))))
	             dp -> priority = nPrty;
		  }
                break;
...
/* Output     : None                                                                */
/* Return     : 0 if OK, otherwise -1                                               */
int uart_shutdown(input_module_t *module, int ms)
{
	return (0);
}
