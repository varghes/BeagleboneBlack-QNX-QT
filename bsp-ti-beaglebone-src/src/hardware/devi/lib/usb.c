...
// Prototypes+
// 
static void insertion( struct usbd_connection *pConnection, usbd_device_instance_t * pInstance );
static void removal( struct usbd_connection *pConnection, usbd_device_instance_t *pInstance );

static void usb_dev_int( struct usbd_urb *urb, struct usbd_pipe *pipe, void *hdl );
...
void *  devi_usb_register_client(input_module_t * pInput_module, int nDev, _uint32 nVendor, 
...
/* Output     : None                                                                */
/* Return     : None                                                                */
void usb_dev_int( struct usbd_urb *pUrb, struct usbd_pipe *pPipe, void *pHandle )
{
   ...
   if( pModule -> flags  & USB_DEVICE_ON )
      {		// re-hook interrupt endpoint
      usbd_setup_interrupt( pModule -> urb, 
			   URB_DIR_IN, 
			   pModule -> ep_int_buf, 
			   pModule -> ep_int_size );
