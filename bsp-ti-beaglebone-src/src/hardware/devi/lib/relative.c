...
/* Output     : None                                                                */
/* Return     : None                                                                */
void load_rel_opts(input_module_t *module)
{
    ...
    */
    ...
    while(fgets( buf, sizeof( buf ), f ))
	 ...
	 if( 0 == strcmp( "-T", opt ) ) // Cursor threshold
	     {
	 ...
	 if( 0 == strcmp( "-l", opt ) ) // Swap buttons
	     {
             dp -> flags |= FLAG_BSWAP;  
	     }
	 else 
	 ...
	 if( 0 == strcmp( "-W", opt ) ) // Wheel parameters
	     {
		...
		sscanf(arg, "%i,%i",  &wheel_gain, &wheel_threshold);
             ...
             if((wheel_threshold > 0) && (wheel_threshold <= MAX_MAX_WHEEL_ACCELERATION))
...
/* Output     : None                                                                */
/* Return     : None                                                                */
void save_rel_opts(input_module_t *module)
{
   struct data *dp = module->data;
   FILE        *f;
...
int rel_init(input_module_t *module)
{
	struct data *dp = module->data;
...
int rel_devctrl(input_module_t *module, int event, void *ptr)
{
	struct data 	*dp = module->data;
	input_module_t	*down = module->down;
	        ...
	        if(NULL != ptr)
	           {
                   ...
                   if((parms->gain > 0) && (parms->gain <= MAX_GAIN))
                      dp->gain = parms->gain;
                   if((parms->threshold > 0) && (parms->threshold <= MAX_CURSOR_THRESHOLD)) 
	 ...
	 case DEVCTL_SETZMODE:
		 ...
		 if(nZmode != 0)
		    {
		    if(nZmode < ZMODE_KEY_UPDOWN ||
		       nZmode > ZMODE_SEND_CLIENT_ONLY)
		         dp -> zmode = nZmode;
		    else
		      return -1;
		    }
...
/* Output     : None                                                                */
/* Return     : 0 if OK, otherwise -1                                               */
int rel_reset(input_module_t *module)
{
	struct data 		*dp = module->data;
	input_module_t		*down = module->down;
        unsigned wheel;
...
int rel_input(input_module_t *module, int num, void *arg)
{
...
/* Output     : None                                                                */
/* Return     : EOK if OK, otherwise -1                                             */
int rel_parm(input_module_t *module,int opt, char *optarg)
{
	struct data *dp = module->data;
        int nVal;
		...
		if(nVal <= MAX_WHEEL_ACCELERATION)
	        ...
	        break;
             ...
             case 'T':
                ...
                if(dp->threshold < 0 || dp->threshold > 255) {
                        dp->threshold = 0;
                } else {
...
int rel_shutdown(input_module_t *module, int delay)
{
	return EOK;
}
...
/* Output     : None                                                                */
/* Return     : None                                                                */
void calculate_cursor_move(struct data * dp,  struct packet_rel *p)
{
   int nCoeff;
   int x = p -> dx,
       y = p -> dy;
   int x0 = x, y0 = y;
   if((dp -> cursor_x >= 0 && x >= 0) ||
      (dp -> cursor_x <= 0 && x <= 0)) 
      dp -> cursor_x += x;
   else
      dp -> cursor_x = 0;
  
   if((dp -> cursor_y >= 0 && y >= 0) ||
      (dp -> cursor_y <= 0 && y <= 0)) 
      dp -> cursor_y += y;
   else
      dp -> cursor_y = 0;
   ...
   /* This coefficient CURSOR_ACCEL_STEP is used in order to get more smooth 
    changing of mouse behaviour while modifying threshold parameter */
       nCoeff = min(CURSOR_ACCEL_STEP, 
		 (abs(dp -> cursor_x) + abs(dp -> cursor_y)) /
		    dp -> threshold);
   ...
   if(verbosity >= 4)
    fprintf(stderr, "x0 = %i, y0 = %i, x(before) = %i, y(before) = %i, thresh = %i, gain = %i, x(new) = %i, y(new) = %i, coeff = %i\n",
	    x0, y0, (int)(dp->cursor_x), (int)(dp -> cursor_y),
	    (int)(dp -> threshold), (int)(dp -> gain), (int)(p->dx), (int)(p->dy), nCoeff);
