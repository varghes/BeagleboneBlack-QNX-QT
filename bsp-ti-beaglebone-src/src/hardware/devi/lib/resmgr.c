...
static int devi_read(resmgr_context_t *ctp, io_read_t *msg, RESMGR_OCB_T *ocb);
...
static int add_wait_queue(devi_attr_t *, devi_ocb_t *, resmgr_context_t *, int);
static int remove_wait_queue(devi_attr_t *, struct _wait_q *);
static int process_wait_queue(devi_attr_t *attr);
static int process_read_data(int, devi_attr_t *attr, devi_ocb_t *ocb, int);
