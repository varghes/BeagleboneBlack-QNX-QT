...
/* Description: function regisers logical port                                      */
...
int register_device(dispatch_t * dp, int (*pulse)(message_context_t *, int, unsigned, void *), int coid, 
		    int nPortNum, void * data)
{
   int nCoid;
   ...
   // usually daes, but it happens onlty when driver is launched without Photon
   // interface); otherwice devi_get_coid failes
   if(DEFAULT_COID == coid)
     glbData.nConnID = devi_get_coid();
   else
     glbData.nConnID = coid;
   ...
   if((0 == nPortNum) && (0 == glbData.irq_base_id))
     ...
     if((glbData.irq_base == glbData.irq_aux) &&
	(glbData.irq_aux_id > 0))
...
//					  _NTO_INTR_FLAGS_END     |
   ...
   if((0 < nPortNum) && (0 == glbData.irq_aux_id))
     { 
     int irq_aux_id;
     if((glbData.irq_base == glbData.irq_aux) &&
	(glbData.irq_base_id > 0))
...
int send_command_set( int nPortNum, unsigned char * pCmds, int nCmdsLen,
		  unsigned char * pReply, int nReplyLen)
{
   ...
   if(get_command_byte(pd, &cmd_byte))
     ...
     for(idCount = 0; idCount < nCmdsLen; ++idCount)
	  ...
	  nOfResends = 0;
       ...
       if(data_byte == RESEND) /* RESEND request */
          {
//	delay(RESEND_DELAY);
...
int init_contr(int flags, int nIrq_base, int nIrq_aux, int nDataPort, int nStatusPort)
{
   ...
   if((glbData.flags & INIT_DONE) == 0)
     ...
     glbData.nDataPort = ((SET_DEFAULT != nDataPort) ? nDataPort  :  DATA_DEF_PORT);
     glbData.nStatPort = ((SET_DEFAULT != nIrq_aux)  ? nStatusPort  :  STATUS_DEF_PORT);
     
     /* get i/o resources                                                */
     glbData.nDataPort_id = devi_request_iorange(glbData.nDataPort, 1, DEVI_SHARE_RSRC);
     if(verbosity >=3)
	 printf("Data port registered (%#x)\n", glbData.nDataPort_id);
     if(glbData.nDataPort_id == (uintptr_t) MAP_FAILED) 
       return (-1);
     glbData.nStatPort_id = devi_request_iorange(glbData.nStatPort, 1, DEVI_SHARE_RSRC);
     if(verbosity >=3)
...
int  switch_controller_mode(int nNewMode)
{
   ...
   if(SET_MULTIPLEX_MODE && (0 == (glbData.flags & MULTIPLEX_MODE)))
     {
     if(0 == glbData.f_swmode(nNewMode))
         glbData.flags |= MULTIPLEX_MODE;
     else
...
{
   unsigned char c;
   int numBlk = ctp -> msg -> pulse.value.sival_int;
