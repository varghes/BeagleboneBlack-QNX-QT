...
{
   send_device(pd, (bEnable ? KBD_ENABLE : KBD_DISABLE), SEND_CONTR, DO_DELAY);
   send_device(pd, (bEnable ? AUX_ENABLE : AUX_DISABLE), SEND_CONTR, DO_DELAY);
}
...
int get_command_byte(pData pd, unsigned char * pCmd_byte)
{
    int rc = EOK;
    ...
    InterruptDisable();
    send_device(pd, GET_CMD_BYTE, SEND_CONTR, NO_DELAY);
...
int get_device_poll(pData pd, int nPortNum, unsigned char * c)
{
   ...
   for(nTries = TRY_LIMIT; 
       (nTries > 0) &&  (pd -> f_chdat(nPortNum, status = in8(pd -> nStatPort_id)));
       nTries--)
     {
...
//     nanospin_ns(pd -> time_delay); 
   ...
   if(nTries == 0)
     {
     if(verbosity >= 5)
        fprintf(stderr, "Polling: no data received. Status byte = %#x, nTries = %i\n", (int)status, (int)TRY_LIMIT);
     return -1;
     }
     ...
     if(-1 == InterruptDetach(nIrq_id))
	  {
          ...
          slogf(_SLOG_SETCODE(_SLOGC_INPUT, 0), _SLOG_ERROR, pMsgTxt, nIrq_id, strerror(errno));
	  }
     }
...
int f_validate_data(int nStatus, int nIrqId)
{
   ...
   if((nIrqId == AUX_IRQ_TYPE) && !(nStatus  & AUX_DATA_COME))
     return RET_NO_DATA;
   else
