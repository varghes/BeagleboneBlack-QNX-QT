                        ...
                        struct sigevent *evp, unsigned flags)
{
	...
	if (!evp) {
                ...
                if (code == DEVI_PULSE_ALLOC) {
                        code = devi_register_pulse(mp, DEVI_PULSE_ALLOC, 
                                                   mp->pulse);
                        if (code < 0) {
 ...
 */
timer_t
devi_register_timer(input_module_t *mp, int prio, int *pc, struct sigevent *evp)
{
	struct sigevent	                ev;
	int				code;
	int				id;
...
devi_request_iorange(unsigned start, unsigned len, unsigned flags)
{
	...
	memset(&res, 0, sizeof(res));
        ...
        if (rsrcdbmgr_attach(&res, 1) < 0) {
                char * pMsgTxt = "Error: failure in requesting iorange (%x, len: %x) from the system\n";
                if(verbosity)
