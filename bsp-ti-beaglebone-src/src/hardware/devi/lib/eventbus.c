...
bus_add(event_bus_line_t *bp, input_module_t *mp)
{
	...
	if (bp->flags & DEVI_LINE_HAVE_PROTO && 
	...
	if (bp->flags & DEVI_LINE_HAVE_DEVICE && 
	    mp->type & DEVI_MODULE_TYPE_DEVICE) {
                char * pMsgTxt = "Error: insertion of module %s into busline that already has a device module\n";
	...
	if (bp->flags & DEVI_LINE_HAVE_FILTER && 
	    mp->type & DEVI_MODULE_TYPE_FILTER) {
                ...
                char * pMsgTxt = "Error: insertion of module %s into busline that already has a filter module\n";
                fprintf(stderr, pMsgTxt, mp->name);
                slogf(_SLOG_SETCODE(_SLOGC_INPUT, 0), _SLOG_ERROR, pMsgTxt, mp->name);
		return (-1);
	}
...
bus_complete(event_bus_line_t *bp)
{
        ...
        if (!(bp->flags & DEVI_LINE_HAVE_DEVICE)) {
                ...
                slogf(_SLOG_SETCODE(_SLOGC_INPUT, 0), _SLOG_ERROR, pMsgTxt, bp->bottom->name);
                return (-1);
        }
	...
	if (bp->top->type & DEVI_CLASS_ABS)
                ...
                char * pMsgTxt = "Error: unable to determine type of filter to add to bus\n";
	...
	if (bp->flags & DEVI_LINE_HAVE_FILTER)
        ...
        if (filter->init(filter) < 0) {
                ...
                slogf(_SLOG_SETCODE(_SLOGC_INPUT, 0), _SLOG_ERROR, pMsgTxt, filter->name);
                return (-1);
        }
