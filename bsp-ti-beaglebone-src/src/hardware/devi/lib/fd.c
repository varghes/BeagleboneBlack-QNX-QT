...
static int fd_pulse(message_context_t *ctx, int code, unsigned flags, void *data);
#else
...
fd_devctrl(input_module_t *module, int event, void *ptr)
{
        ...
        case DEVCTL_RESET: 
                ...
                if(dp->flags & FLAG_SER) {
                        ...
                        for(i = 0; i < 2; i++) {
                                ...
                                devctl(dp->fd, DCMD_CHR_SERCTL, &sw, 
                                       sizeof(sw), NULL);
        ...
        case DEVCTL_SETBAUD:
                ...
                if (dp->flags & FLAG_SER) {
                        ...
                        tios.c_cflag = (tios.c_cflag & ~CLOCAL) | 
                                portctl->cflag;
        ...
        case DEVCTL_LED: {
                ...
                if (dp->flags & FLAG_KBD) {
                        ...
                        flag |= _CONCTL_SCROLL_CHG | _CONCTL_NUM_CHG | 
                                _CONCTL_CAPS_CHG;
                        if (*led & KEYIND_SCROLL_LOCK) flag |= _CONCTL_SCROLL;
                        if (*led & KEYIND_CAPS_LOCK) flag |= _CONCTL_CAPS;
                        if (*led & KEYIND_NUM_LOCK) flag |= _CONCTL_NUM;
                        
                        if (devctl(dp->fd, DCMD_CHR_SERCTL, &flag, 
                                   sizeof(flag), 0) != 0)
                                perror("devctl");
                        break;
                }
...
fd_reset(input_module_t *module)
{
        ...
        if((dp->flags & FLAG_INIT) == 0) {
                ...
                if(!(dp->flags & FLAG_OPENED)) {
                        ...
                        if((dp->fd = open(dp->dev_name, O_RDWR)) == -1) {
                                ...
                                slogf(_SLOG_SETCODE(_SLOGC_INPUT, 0), _SLOG_ERROR, pMsgTxt, dp->dev_name);
                                return (-1);
                        }
		...
		dp->pulse_code = devi_register_pulse(module, DEVI_PULSE_ALLOC,
						       fd_pulse);
...
#else
                if (pthread_create(&dp->tid, NULL, fd_thread, 
                                   (void *) module) != EOK) {
	...
	int			count;
        ...
        for (;;) {
	        if(dp -> priority != priority) /* priority was changed                      */
	           {
                   dp -> priority = priority = min(dp -> priority, sched_get_priority_max(sched_getscheduler(0)));
		   setprio(0, priority);
		   }
                count = read(dp->fd, buffer, sizeof(buffer));
                if (count < 0) {
