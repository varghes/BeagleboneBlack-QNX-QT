...
// JOYSTICK
typedef struct _rep_joystick_data
{
  joystick_data_t joystickData;
} rep_joystick_data_t, * pRep_joystick_data_t;
...
static LIST_HEAD(_modList,  _module_data) modList;// List of the registred modules
...
static void report(struct hidd_connection *, struct hidd_report *handle, void *report_data, 
...
static void attach_keyboard_reports(struct hidd_connection * pConnection, hidd_device_instance_t * pInstance, 
				   struct hidd_collection * pCollection);
static void attach_mouse_reports(struct hidd_connection * pConnection, hidd_device_instance_t * pInstance, 
				   struct hidd_collection * pCollection);
static void attach_joystick_reports(struct hidd_connection * pConnection, hidd_device_instance_t * pInstance, 
				   struct hidd_collection * pCollection);
static void attach_touch_reports(struct hidd_connection * pConnection, hidd_device_instance_t * pInstance, 
				   struct hidd_collection * pCollection);

static int accept_report(struct hidd_collection * pCollection, struct hidd_device_instance * pDevInstance, 
		  _uint16 nRepIndex, _uint16 nRepType, _uint16 nConnType, pReport_data_t * ppRepData);
static int attach_input_reports(pModule_data_t pModule, hidd_device_instance_t * pInstance, 
			  struct  hidd_collection * pCollection, _uint16 nRepClass, void * pPrivData);

static void report_keyboard(struct hidd_report * pReport, void * pReportData, _uint32 nRepLen, _uint32 flags, 
		     pReport_data_t pPrivData);
static void report_mouse(struct hidd_report * pReport, void * pReportData, _uint32 nRepLen, _uint32 flags, 
		     pReport_data_t pPrivData);
static void report_joystick(struct hidd_report * pReport, void * pReportData, _uint32 nRepLen, _uint32 flags, 
		     pReport_data_t pPrivData);
static void report_touch(struct hidd_report * pReport, void * pReportData, _uint32 nRepLen, _uint32 flags, 
		     pReport_data_t pPrivData);
...
static int is_mouse_report(hidd_device_instance_t * pInstance, 
...
{
   int rc = EOK;
   hidd_connect_parm_t hparm = { NULL, HID_VERSION, HIDD_VERSION, 0, 0, 0, 0, 0, 0, HIDD_CONNECT_WAIT };
   
   hidd_funcs_t hf = {_HIDDI_NFUNCS, 
	               insertion, 
	               removal, 
	               report, 
	               event  };
     ...
     switch(usage_page)
	  ...
	  case HIDD_PAGE_DESKTOP:
		...
		case HIDD_USAGE_POINTER:
		  if(is_mouse_report(pInstance, pCollections[i], &is_mouse) && !is_mouse )
		     {
		     // This doesn't look nice: I don't find any standard touch screen
		     // report descripti. On the contrary, I found an example of Semtech 
...
/* Comment    : None                                                                */
int accept_report(struct hidd_collection * pCollection, struct hidd_device_instance * pDevInstance, 
		  _uint16 nRepIndex, _uint16 nRepType, _uint16 nConnType, pReport_data_t * ppRepData)
		  
{
     ...
     if( EOK == (rc = hidd_report_attach( pConnection, pDevInstance, pRepInstance,
				    nConnType,
				    nRepLen + sizeof(report_data_t), &pReport )))
         {
	 if(verbosity >=3)
	      printf("Report attach: pReport = %p, report instance = %p, report data = %p\n", pReport, pRepInstance,
		*ppRepData);
     ...
     if(EOK != rc)
        {
	fprintf(stderr, "hidd_report_attach faied (%i)\n", rc);
        return rc;
	}
...
/* Comment    : None                                                                */
int attach_input_reports(pModule_data_t pModule, hidd_device_instance_t * pInstance, 
			  struct  hidd_collection * pCollection, _uint16 nRepClass, void * pPrivData)
{
...
/* Return     : None                                                                */
/* Comment    : None                                                                */
void	report(struct hidd_connection * pConnection, struct hidd_report *handle, void *report_data, 
	       _uint32 report_len, _uint32 flags, void *user)
{
   ...
   if(verbosity >= 3)
     printf("Report comes - report handle %p, report_data %p\n", handle, pRepData);
...
/* Return     : None                                                                */
/* Comment    : None                                                                */
void attach_keyboard_reports(struct hidd_connection * pConnection, hidd_device_instance_t * pInstance, 
			    struct  hidd_collection * pCollection)
{
   ...
   for(pModule = LIST_FIRST_ITEM(&modList); NULL != pModule; pModule = LIST_NEXT_ITEM(pModule, lst_conn))
     ...
     if(EOK != (rc = attach_input_reports(pModule, pInstance, pCollection, HIDD_KEYBOARD_REPORT, pKeyboardData)))
	{
	...
	return;
     ...
     pModule -> pInput_module -> devctrl(pModule -> pInput_module, DEVCTL_GETKBD, &devctl);
...
/* Return     : None                                                                */
/* Comment    : None                                                                */
void attach_mouse_reports(struct hidd_connection * pConnection, hidd_device_instance_t * pInstance, 
			 struct hidd_collection * pCollection)
{
   pModule_data_t pModule;   // List of the registred modules
   ...
   for(pModule = LIST_FIRST_ITEM(&modList); NULL != pModule; pModule = LIST_NEXT_ITEM(pModule, lst_conn))
    ...
    if(verbosity >= 3)
     ...
     if(NULL == (pMouseData = (pRep_mouse_data_t)alloc_device_data(pModule, pInstance)))
	  {
	  if(verbosity)
	       fprintf(stderr, "Error: not enough memory\n");
	  return;
     ...
     // And attach all input report
     if(EOK != (rc = attach_input_reports(pModule, pInstance, pCollection, HIDD_MOUSE_REPORT, pMouseData)))
	{
	  ...
	  if(0 != nButtons)
	       {
	        ...
	        if(verbosity >=3)
	             printf("Mouse has %i buttons\n", (int)nButtons);
		break;
	       }
	  }
	
     pMouseData -> flags = 0;
     if((EOK == (rc = hidd_get_protocol(pConnection, pInstance, &nProtocolId))) &&
...
/* Return     : None                                                                */
/* Comment    : None                                                                */
void attach_joystick_reports(struct hidd_connection * pConnection, hidd_device_instance_t * pInstance, 
			    struct hidd_collection * pCollection)
{
...
/* Return     : None                                                                */
/* Comment    : None                                                                */
void attach_touch_reports(struct hidd_connection * pConnection, hidd_device_instance_t * pInstance, 
			    struct  hidd_collection * pCollection)
{
   pModule_data_t pModule;   // List of the registred modules
   pReport_data_t 
   ...
   for(pModule = LIST_FIRST_ITEM(&modList); NULL != pModule; pModule = LIST_NEXT_ITEM(pModule, lst_conn))
     ...
     if(!(pModule -> pInput_module -> type & DEVI_CLASS_ABS))
	  continue;
     if(verbosity >= 3)
        printf("Attach touch screen report\n");
     ...
     if(EOK == accept_report(pCollection, pInstance, 0, HID_FEATURE_REPORT, 0, //HIDD_REPORT_EXCLUSIVE,
			     &pFeatureRepData))
	  {
          ...
          pFeatureRepData -> usage         = HIDD_USAGE_UNDEFINED;
          pFeatureRepData -> pModule       = pModule;
          pFeatureRepData -> pPrivData     = NULL;
          LIST_INSERT_HEAD(&(pModule -> featureRepList), pFeatureRepData, lst_conn);
	  }

     // And attach all input report
     if(EOK != (rc = attach_input_reports(pModule, pInstance, pCollection, HIDD_TOUCHSCREEN_REPORT, NULL)))
	{
...
/* Return     : None                                                                */
/* Comment    : None                                                                */
void report_keyboard(struct hidd_report * pReport, void * pReportData, _uint32 nRepLen, _uint32 flags, 
		     pReport_data_t pPrivData)
{
   ...
   if((verbosity  >= 3) && (0 != nKeys))
     {
     printf("keys = %i, pressed = %i, released = %i, keys:", nKeys, nPressed, nReleased);
     for(i = 0; i < nKeys; ++i)
       {
   ...
   if(pKbdData -> nLastPressed >= 0) // If previous "last pressed" is still pressed, update its index
     {
    ...
    if(nPressed > 0)
     {
	   ...
	   sizeof(pressed[0]) * (nPressed + 1), (void *)pressed);
     }
    if(nReleased > 0)
     {
       (pInput_module -> input)(pInput_module, 
	   sizeof(released[0]) * (nReleased + 1), (void *)released);
     }
...
/* Return     : None                                                                */
/* Comment    : None                                                                */
void report_mouse(struct hidd_report * pReport, void * pReportData, _uint32 nRepLen, _uint32 flags, 
		     pReport_data_t pPrivData)
{
   pRep_mouse_data_t pMouseData;
   ...
   if(EOK == hidd_get_usage_value( pPrivData -> pRepInstance, NULL, HIDD_PAGE_DESKTOP, HIDD_USAGE_X, 
				  pReportData, &nValue))
      mouseRawData.x = (_int16)nValue;
   else
      mouseRawData.x = 0;
   if(EOK ==  hidd_get_usage_value( pPrivData -> pRepInstance, NULL,  HIDD_PAGE_DESKTOP, HIDD_USAGE_Y, 
				  pReportData, &nValue))
   ...
   if((pMouseData -> flags  & HID_MOUSE_WHEEL_ON) &&
      ((EOK == hidd_get_usage_value( pPrivData -> pRepInstance, NULL,  HIDD_PAGE_DESKTOP, HIDD_USAGE_WHEEL,
				  pReportData, &nValue)) ||
       (EOK == hidd_get_usage_value( pPrivData -> pRepInstance, NULL,  HIDD_PAGE_DESKTOP, HIDD_USAGE_Z,
				  pReportData, &nValue))))
     {
...
/* Return     : None                                                                */
/* Comment    : None                                                                */
void report_joystick(struct hidd_report * pReport, void * pReportData, _uint32 nRepLen, _uint32 flags, 
		     pReport_data_t pPrivData)
{
   pReport = pReport, pReportData = pReportData, nRepLen = nRepLen, flags = flags, 
		     pPrivData = pPrivData;
}
...
/* Return     : None                                                                */
/* Comment    : None                                                                */
void report_touch(struct hidd_report * pReport, void * pReportData, _uint32 nRepLen, _uint32 flags, 
		     pReport_data_t pPrivData)
{
   _uint32 nValue;
   ...
   // Is there pointer data?
   if(EOK != hidd_get_usage_value( pPrivData -> pRepInstance, NULL, HIDD_PAGE_DIGITIZER, HIDD_USAGE_X, 
				  pReportData, &nValue))
      return;
   touchRawData.x = (_int16)nValue;
   if(EOK !=  hidd_get_usage_value( pPrivData -> pRepInstance, NULL,  HIDD_PAGE_DIGITIZER, HIDD_USAGE_Y, 
				  pReportData, &nValue))
     return;
   ...
   if(verbosity >= 3)
     {
     printf("Raw touch screen data: %s, x = %d, y = %d\n", 
   ...
   // structure (see hid.h)
   pInput_module = pPrivData -> pModule -> pInput_module;
   (pInput_module -> input)(pInput_module, sizeof(touchRawData),
			    (void *) &touchRawData);
...
/* Comment    : None                                                                */   
int devi_hid_devctrl(void * pHandle, int event, void *ptr, int nDev)
{
   ...
   for(pDeviceData = LIST_FIRST_ITEM(&(pModule -> devDataList)); NULL != pDeviceData; pDeviceData = LIST_NEXT_ITEM(pDeviceData, lst_conn))
     {
     if((nDev != HIDD_CONNECT_WILDCARD) && (nDev != pDeviceData -> pDevInstance -> devno)) 
...
/* Comment    : None                                                                */   
int kbd_devctrl(pModule_data_t pModule, int event, void *ptr, void * pPrivData)
{
    int rc = EOK;
    ...
    switch(event) 
	   ...
	   if(NULL != ptr)
	     {
	     _uint16 anLedFlags[] = { KEYIND_SCROLL_LOCK, KEYIND_NUM_LOCK, KEYIND_CAPS_LOCK  };
	     _uint16 anLedUsages[] = { HIDD_USAGE_SCROLL_LOCK, HIDD_USAGE_NUM_LOCK, HIDD_USAGE_CAPS_LOCK  };
	     int i;
		 ...
		 {
                 if(EOK != (rc = hidd_report_len( pLedReport -> pRepInstance, &nRepLen)))
		    break;
	         ...
	         for(i = 0; i < ARRAY_SIZE(anLedFlags); ++i)
		    {
	            rc = hidd_set_usage_value(pLedReport -> pRepInstance, NULL, //pLedReport -> pCollection, 
				  HIDD_PAGE_LEDS, anLedUsages[i], 
				     (nLeds & anLedFlags[i]) ? 1 : 0, pRepData, nRepLen);
		    }
...
/* Return     : EOK if OK, otherwise error code                                     */
/* Comment    : None                                                                */   
int mouse_devctrl(pModule_data_t pModule, int event, void *ptr, void * pPrivData)
{
    int rc = EOK;
...
void detach_reports(pReport_data_t pRepData)
{
...
void remove_device_reports(hidd_device_instance_t * pDevInstance)
{
   pModule_data_t pModule;   // List of the registred modules
   ...
   for(pModule = LIST_FIRST_ITEM(&modList); NULL != pModule; pModule = LIST_NEXT_ITEM(pModule, lst_conn))
     ...
     remove_device_reports_from_list(LIST_FIRST_ITEM(&(pModule -> inpRepList)), pDevInstance);
     if(verbosity >= 3)
         printf("Remove output report - %p\n", LIST_FIRST_ITEM(&(pModule -> outRepList)));
     remove_device_reports_from_list(LIST_FIRST_ITEM(&(pModule -> outRepList)), pDevInstance);
     if(verbosity >= 3)
         printf("Remove feature report - %p\n", LIST_FIRST_ITEM(&(pModule -> featureRepList)));
     remove_device_reports_from_list(LIST_FIRST_ITEM(&(pModule -> featureRepList)), pDevInstance);
     }
}

void remove_device_reports_from_list(pReport_data_t pFirstRepData, hidd_device_instance_t * pDevInstance)
{
...
void remove_device_data(hidd_device_instance_t * pDevInstance)
{
   pModule_data_t pModule;   // List of the registred modules
   ...
   for(pModule = LIST_FIRST_ITEM(&modList); NULL != pModule; pModule = LIST_NEXT_ITEM(pModule, lst_conn))
     {
     remove_device_data_from_list(LIST_FIRST_ITEM(&(pModule -> devDataList)), pDevInstance);
     }
}

void remove_device_data_from_list(pDevice_data_t pFirstDeviceData, hidd_device_instance_t * pDevInstance)
{
...
/* Comment    : None                                                                */
int is_mouse_report(hidd_device_instance_t * pInstance,  struct hidd_collection * pCollection, int * pIsMouse)
{
   ...
   for(i = 0; EOK == hidd_get_report_instance(pCollection, i, HID_INPUT_REPORT, &pRepInstance ); ++i)
       {
       if((EOK != hidd_get_num_props(pRepInstance, &nNumProps)) || (0 == nNumProps))
	    continue; /* We cannot determine anything */
       nPropsLen = sizeof(hidd_report_props_t) * nNumProps;
       pReport_props = malloc(nPropsLen);
       if(EOK == hidd_get_report_props(pRepInstance, pReport_props, &nPropsLen))
       for(j = 0; j < nNumProps; ++j)
	    {
	    if((pReport_props[j].usage_page == HIDD_PAGE_DESKTOP) && 
	       (pReport_props[j].usage_min <= HIDD_USAGE_X) && 
	       (pReport_props[j].usage_max >= HIDD_USAGE_Y))
		 {
		 *pIsMouse =  (0 == (pReport_props[j].data_properties & HIDD_DATA_RELATIVE));
		 free(pReport_props);
		 return 1;
		 }
	    }
