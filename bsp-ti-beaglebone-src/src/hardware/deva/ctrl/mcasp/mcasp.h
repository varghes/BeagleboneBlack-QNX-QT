...
#define AFSRCTL_RMOD_I2S	(2<<7)		/* 2-slot i2s mode */
...
#define AFSXCTL_XMOD_I2S	(2<<7)		/* 2-slot i2s mode */
...
typedef struct EDMA3
{
   ...
   vuint32_t DREAH0;		/* 0x344 */
   vuint32_t DRAE1;			/* 0x348 */
   vuint32_t DREAH1;		/* 0x34c */
   vuint32_t DRAE2;			/* 0x350 */
   vuint32_t DREAH2;		/* 0x354 */
   vuint32_t DRAE3;			/* 0x358 */
   vuint32_t DREAH3;		/* 0x35c */
   uint8_t res_g[32];
...
struct mcasp_card {
	...
	uint32_t			mcasp_dmax;
	uint32_t			dma_play_idx;	
	uint32_t			dma_cap_idx;
	uint8_t				i2c_dev;
	uint8_t				tx_voices;
	uint8_t				rx_voices;
	uint32_t			clk_rate;
...
/* aic23.c */
int aic23_mixer ( ado_card_t *card , uint32_t sample_rate , uint32_t i2c_addr, int clk_master,  ado_mixer_t **mixer );
