...
static int mx35_clock(SIM_HBA *hba, int *clock, int high_speed)
{
	...
	for (pre_div = 1; pre_div < 256; pre_div *= 2) {
		if ((max_clk / pre_div) <= (*clock))
			break;
	}

	*clock = max_clk / pre_div;
	pre_div >>= 1;
...
__SRCVERSION( "$URL: http://svn/product/tags/restricted/bsp/nto650/ti-j5-evm/latest/src/hardware/devb/mmcsd/chipsets/sim_mx35.c $ $Rev: 600259 $" );
