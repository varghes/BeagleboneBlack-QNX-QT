...
mmc_creat_threads(SIM_HBA *hba)
{
	...
	if ((hba->simq = simq_init(hba->coid, hba, 1, MAX_LUN, 2, 1, 2, 8)) == NULL) {
	...
	pthread_attr_init(&attr);
	pthread_attr_setschedpolicy(&attr, SCHED_RR);
	param.sched_priority = MMC_PRIORITY;
	pthread_attr_setschedparam(&attr, &param);
	pthread_attr_setinheritsched(&attr, PTHREAD_EXPLICIT_SCHED);
	pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
	pthread_attr_setstacksize(&attr, MMC_STACK_SIZE);
...
__SRCVERSION( "$URL: http://svn/product/tags/internal/bsp/nto650/ti-j5-evm/1.0.0/latest/hardware/devb/mmcsd/sim.c $ $Rev: 600251 $" );
