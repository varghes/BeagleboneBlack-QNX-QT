...
static const struct sigevent *spi_intr(void *area, int id)
{
	...
	if(reg_value & (INTR_TYPE_RX0_FULL << (channel_id * OMAP3_INTERRUPT_BITS_PER_SPI_CHANNEL))) {
			expected = min(OMAP3_SPI_FIFOLEN, dev->tlen - dev->rlen);
			// Read received block
			for(i = 0 ; i < expected; i++) {
					dev->pbuf[dev->rlen++] = in32(base + OMAP3_MCSPI_CH1_RX_BUFFER_OFFSET + (OMAP3_SPI_DEVICE_OFFSET * channel_id));
			}
		/*
		 * Sometimes, Tx empty interrupt is not triggered (Hardware issue?). Therefore, we start writing to tx register whenever
		 * Rx Full interrrupt is raised,  to make sure all the data can be transferred
		*/
		expected = min(OMAP3_SPI_FIFOLEN, dev->xlen - dev->tlen);
		if(expected) {
			// Start Tx again
			for(i = 0; i < expected; ++i) {
				out32(base + OMAP3_MCSPI_CH1_TX_BUFFER_OFFSET + (OMAP3_SPI_DEVICE_OFFSET * channel_id), dev->pbuf[dev->tlen++]);
			}			
		}
	}
...
#if 0   
	// Is Tx empty?
	if(reg_value & (INTR_TYPE_TX0_EMPTY << (channel_id * OMAP3_INTERRUPT_BITS_PER_SPI_CHANNEL))) { 
		expected = min(OMAP3_SPI_FIFOLEN, dev->xlen - dev->tlen);
		if(expected) {
			// Start Tx again
			for(i = 0; i < expected; ++i) {
...
int omap3_attach_intr(omap3_spi_t *omap3)
{
	...
	omap3->spievent.sigev_priority = OMAP3_SPI_PRIORITY;
...
int omap3_edma_attach_intr(omap3_spi_t *omap3)
{
	...
	event.sigev_priority = OMAP3_SPI_PRIORITY;
